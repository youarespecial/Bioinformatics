...
USED=which(BATCH %in% c('deci1-NM' ,'deci2-NM', 'deci3-NM', 'deci4-PE', 'deci5-NM', 'deci6-PE', 'deci7-NM', 'deci8-PE','deci9-PE'))
...
##########################
mybeer=BEER(DATA, BATCH, GNUM=30, PCNUM=200, ROUND=1, GN=2000, SEED=1, COMBAT=TRUE, RMG=NULL)   
...
pbmc <- RunUMAP(object = pbmc, reduction.use='pca',dims = 1:100, check_duplicates=FALSE,
                n.neighbors = 10, min.dist=0.1)
pdf('2020_0110hfz.dimplot.pc200.neib10.dim10.pdf')
DimPlot(pbmc, reduction.use='umap', group.by='batch', pt.size=0.1,label=F)
dev.off()

pdf('2020_0110hfz.pc200.neib10.dim100.featureplot.CCL5.pdf')
FeaturePlot(pbmc,features='CCL5',order=TRUE)
dev.off()
pdf('2020_0110hfz.pc200.neib10.dim100.featureplot.KRT7.pdf')
FeaturePlot(pbmc,features='KRT7',order=TRUE)
dev.off()

pdf('2020_0110hfz.pc200.neib10.dim100.featureplot.CD34.pdf')
FeaturePlot(pbmc,features='CD34',order=TRUE)
dev.off()

pdf('2020_0110hfz.pc200.neib10.dim100.featureplot.GNLY.pdf')
FeaturePlot(pbmc,features='GNLY',order=TRUE)
dev.off()
...
USED=which(BATCH %in% c('pla7-NM' ,'pla7-NM' ,'pla7-NM' ,'pla7-NM' ,'pla7-NM' ,'pla7-NM' ,'pla7-NM' ,'pla7-NM'))
...
pdf('2020_0110hfz.pca50.umapdim50.pdf')
DimPlot(pbmc, reduction = "umap")
dev.off()
# saveRDS(pbmc,'pbmc_decidua.RDS')
########        adjust          ##############
pbmc <- RunPCA(pbmc, features = VariableFeatures(object = pbmc),npcs = 50)
pbmc <- RunUMAP(pbmc, dims = 1:150)
pdf('2020_0110hfz.pca50.umapdim150.pdf')
DimPlot(pbmc, reduction = "umap")
dev.off()
# saveRDS(pbmc,'pbmc_decidua.RDS')
...
pbmc <- RunPCA(pbmc, features = VariableFeatures(object = pbmc),npcs = 150)
pbmc <- RunUMAP(pbmc, dims = 1:150)
pdf('2020_0110hfz.pca150.umapdim150.pdf')
DimPlot(pbmc, reduction = "umap")
dev.off()
# saveRDS(pbmc,'pbmc_decidua.RDS')
########        adjust          ##############
pbmc <- RunPCA(pbmc, features = VariableFeatures(object = pbmc),npcs = 150)
pbmc <- RunUMAP(pbmc, dims = 1:50)
pdf('2020_0110hfz.pca150.umapdim50.pdf')
DimPlot(pbmc, reduction = "umap")
dev.off()
# saveRDS(pbmc,'pbmc_decidua.RDS')
########        adjust          ##############
